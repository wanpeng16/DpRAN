...
class Non_Local_Co(nn.Module):
    """Co-learning with Non-local TA"""
    ...
    def __init__(self, in_channels_1, in_channels_2, out_channels):
        ...
        self.fc_q = nn.Linear(in_channels_1 * 2, in_channels_1, bias=False)
        self.fc_k = nn.Linear(in_channels_1 * 2, in_channels_1, bias=False)
        self.fc_v = nn.Conv2d(in_channels_1 * 2, in_channels_1 * 2, kernel_size=(1, 1), stride=(1, 1), bias=False)
    ...
    def forward(self, x1, x2, x3):
        ...
        out = self.non_local(co_learned.permute(1, 0, 2, 3, 4))
        if x3 is not None:
            x3 = self.up(x3)
            # input is CHW
...
if __name__ == "__main__":
    ...
    m = Non_Local_Co(in_channels_1=32, in_channels_2=64, out_channels=32)
